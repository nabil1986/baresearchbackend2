...
app.put('/devices/:id', (req, res) => {
  ...
  const query = 'UPDATE devices SET device_name = ?, grease_quantity = ?, grease_period = ?, observation = ?, niveau = ?, numero_inventaire = ?, designation_grade_graisse = ?, date_prochain_graissage = ?, ordre_passage = ?, equipement_localisation = ?, tempsGraissage = ?, photo = ?, gamme = ?, designation_grade_huile = ?, etage = ? WHERE id = ?';
  db.query(query, [device_name, grease_quantity, grease_period, observation, niveau, numero_inventaire, designation_grade_graisse, dateProchainGraissage, ordre_passage, equipement_localisation, tempsGraissage, photo, gamme, designation_grade_huile, etage, id], (err, result) => {
    if (err) {
      res.status(500).send(err);
    } else {
